...
// The google-protobuf message declaration
EGE_BEGIN_NAMESPACE_2( google, protobuf )
	class MessageLite;
EGE_END_NAMESPACE_2( )
...
REF_OBJECT_DECL(IMeasurableObject);
...
// Static with interface name macros
#define RTTI_INTERFACE( I ) \
public: \
	static const AStringObj&	RTTI_InterfaceTypeNameA( ) { STATIC_ASTROBJ_V( x, typeid( I ).name( ) ); return ASTROBJ( x ); } \
	static const WStringObj&	RTTI_InterfaceTypeNameW( ) { STATIC_WSTROBJ_V( x, WString( ).FromString( _ENCODING_ANSI, typeid( I ).name( ) ) ); return WSTROBJ( x ); } \
	static _dword				RTTI_InterfaceTypeID( ) { STATIC_ASTROBJ_V( x, typeid( I ).name( ) ); return ASTROBJ( x ).GetID( ); }

// Static with interface and class name macros
#define RTTI_CLASS( x ) \
public: \
	static _dword				RTTI_ClassTypeID( ) { return RTTI_ClassNameA( ).GetID( ); } \
	static const AStringObj&	RTTI_ClassNameA( ) { STATIC_ASTROBJ_V( x, #x ); return ASTROBJ( x ); } \
	static const WStringObj&	RTTI_ClassNameW( ) { STATIC_WSTROBJ_V( x, _WIDEN( #x ) ); return WSTROBJ( x ); } \
	static const AStringObj&	RTTI_InterfaceNameA( ) { return RTTI_InterfaceTypeNameA( ); } \
	static const WStringObj&	RTTI_InterfaceNameW( ) { return RTTI_InterfaceTypeNameW( ); } \
public: \
	virtual const AStringObj&	GetInterfaceTypeNameA( ) const { return RTTI_InterfaceNameA( ); } \
	virtual const WStringObj&	GetInterfaceTypeNameW( ) const { return RTTI_InterfaceNameW( ); } \
	virtual const AStringObj&	GetClassTypeNameA( ) const { return RTTI_ClassNameA( ); } \
	virtual const WStringObj&	GetClassTypeNameW( ) const { return RTTI_ClassNameW( ); }

// Class creation
#define RTTI_CLASS_CREATION( x )			static IObject* RTTI_Create( ) { return new x( ); }
#define RTTI_CLASS_CREATION_ARGS( x, t )	static IObject* RTTI_CreateWithArguments( _void* arguments ) { return new x( (t)arguments ); }

// Class creation register macros
#define RTTI_CLASS_CREATION_REGISTER( x ) { \
	STATIC_ASTROBJ_V( x, #x ); \
	GetInterfaceFactory( )->RegisterObjectCreationFunc( ASTROBJ( x ), x::RTTI_Create ); \
	GetInterfaceFactory( )->RegisterObjectCreationFunc( ASTROBJ( x ).GetID( ), x::RTTI_Create ); \
	}
#define RTTI_CLASS_CREATION_ARGS_REGISTER( x ) { \
	STATIC_ASTROBJ_V( x, #x ); \
	GetInterfaceFactory( )->RegisterObjectCreationFunc( ASTROBJ( x ), x::RTTI_CreateWithArguments ); \
	GetInterfaceFactory( )->RegisterObjectCreationFunc( ASTROBJ( x ).GetID( ), x::RTTI_CreateWithArguments ); \
	}
...
#include "Interface/Engine/EngineSerialization.h"
