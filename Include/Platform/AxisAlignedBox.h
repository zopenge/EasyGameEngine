...
public:
	...
	inline _ubool Equal( const AxisAlignedBox& box, _float margin = Math::cFloatEpsilon ) const;
...
public:
	...
	static AxisAlignedBox Union( const AxisAlignedBox& box1, const AxisAlignedBox& box2 );
};
...
AxisAlignedBox::AxisAlignedBox( _float x1, _float y1, _float z1, _float x2, _float y2, _float z2 )
...
AxisAlignedBox::AxisAlignedBox( const Vector3& minvector, const Vector3& maxvector )
{
	if ( minvector.x < maxvector.x )
		{ mMinVector.x = minvector.x; mMaxVector.x = maxvector.x; }
	else
		{ mMinVector.x = maxvector.x; mMaxVector.x = minvector.x; }

	if ( minvector.y < maxvector.y )
		{ mMinVector.y = minvector.y; mMaxVector.y = maxvector.y; }
	else
		{ mMinVector.y = maxvector.y; mMaxVector.y = minvector.y; }

	if ( minvector.z < maxvector.z )
		{ mMinVector.z = minvector.z; mMaxVector.z = maxvector.z; }
	else
		{ mMinVector.z = maxvector.z; mMaxVector.z = minvector.z; }
}
...
{
	// Thanks for sharing the cool code : http://dev.theomader.com/transform-bounding-boxes/
	...
	mMinVector = Vector3::Min(xa, xb) + Vector3::Min(ya, yb) + Vector3::Min(za, zb) + translation;
	mMaxVector = Vector3::Max(xa, xb) + Vector3::Max(ya, yb) + Vector3::Max(za, zb) + translation;
...
{
	Box box( mMinVector, mMaxVector ); box *= trasform;
