...
class Camera {
 ...
 public:
  ...
  Camera(const Vector3& position, const Vector3& look, const Vector3& up,
         _float fovdegree = 58.0f);
 ...
 public:
  ...
  static ThirdPersonCamera Lerp(const ThirdPersonCamera& camera1,
                                const ThirdPersonCamera& camera2, _float alpha);
};
...
_ubool Camera::operator==(const Camera& camera) const {
  return mPosition == camera.mPosition && mLook == camera.mLook &&
         mUp == camera.mUp && mRight == camera.mRight &&
