...
{
	return Math::Equal( mPitch, angle.mPitch ) && Math::Equal( mHeading, angle.mHeading ) && Math::Equal( mBank, angle.mBank );
}
...
{
	return EulerAngles( mPitch + vector.mPitch, mHeading + vector.mHeading, mBank + vector.mBank );
}
