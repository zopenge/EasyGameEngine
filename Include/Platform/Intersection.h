...
public:
	...
	static _ubool RayVSAABox(const Ray& ray, const Bounds& box, _float* distance = _null);
	//! Ray VS mesh.
	//! @param ray    The ray.
	...
	static _ubool AABoxVSAABox(const Bounds& box1, const Bounds& box2);
	//! Bound-box VS plane.
	//! @param box   The bound-box.
	//! @param plane  The plane.
	//! @return True indicates bound-box hit plane successful.
	static _ubool AABoxVSPlane(const Bounds& box, const Plane& plane);

	static _ubool AABoxVSSphere(const Bounds& box, const Sphere& sphere);
