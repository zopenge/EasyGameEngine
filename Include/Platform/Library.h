...
public:
	...
	//!	@return		The dll module handle when we handled the DLL loading procession.
	typedef _handle (*OnLoadDependentDLL)( AStringPtr dllfilename, const QwordParameters2& parameters );

private:
	typedef CallbackFunc< OnLoadDependentDLL, _handle, QwordParameters2 > LoadDependentDLLFunc;
...
private:
	...
	//!	@param		address			The DLL embedded manifest info address.
	//!	@param		size			The DLL embedded manifest info size in bytes.
	//!	@return		True indicates the library had been loaded.
	...
	//!	@param		base			The base image resource directory.
	//!	@param		resource		The current image resource directory.
	//!	@return		True indicates the library had been loaded.
...
public:
	...
	//!	@param		funcpointer		The load dependent libraries function pointer.
	//!	@param		parameters		The user defined parameters.
	//! @return		True indicates success, false indicates failure.
	_ubool Load( const _byte* buffer, _dword size, OnLoadDependentDLL funcpointer, const QwordParameters2& parameters );
	//!	Free library.
	//!	@param		none.
	//! @return		none.
	_void Free( );
