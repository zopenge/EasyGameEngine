...
public:
	...
	~Lock( )
	{
		Platform::DeleteCriticalSection( mSectionHandle );
	}
...
public:
	...
	_void Enter( ) const
	{
		...
		if ( Platform::IsMainThread( ) )
			lock_time = Platform::GetCurrentTickCount( );
#endif
	...
	_void Leave( ) const
	{
		Platform::LeaveCriticalSection( mSectionHandle );
	}
};
...
public:
	...
	LockOwner( const Lock& lock ) : mLock( lock )
	{
		mLock.Enter( );
	}
