...
class Math {
 ...
 public:
  ...
  //! @param min   Minimal value of the range.
  //! @param max   Maximal value of the range.
  //! @return Returns the wrapped value between <b>min</b> and
  //! <b>max</b>.    Such that <b>min</b> = 1, <b>max</b> = 5
  //! then passing in 7 will return a value of 2.
  template <typename Type>
  ...
  //! @param clamp  True indicates would clamp the value by
  //! the input range.
  //! @return Returns the remapped value.
  template <typename Type>
  static Type Remap(Type value, Type input_min, Type input_max, Type output_min,
                    Type output_max, _ubool clamp = _true);
  ...
  //! between 0 to 1.
  //! @return The result value of interpolation, value1 + ( value2 -
  //! value1
  //! )
  //! * factor.
  template <typename Type>
 ...
 public:
  ...
  static _ubool Equal(_float v1, _float v2,
                      _float margin = Math::cFloatEpsilon);
  //! Compare two value with double margin.
  //! @param v1   The value 1.
  //! @param v2   The value 2.
  //! @param margin  The margin that allows some error.
  //! @return True if they are identical, false otherwise.
  static _ubool Equal(_double v1, _double v2,
                      _double margin = Math::cDoubleEpsilon);
  ...
  //! @param value  Floating-point value.
  //! @return A float value representing the smallest integer that is
  //! greater than or equal to argument.
  static _float Ceil(_float value);
  //! Calculates the floor of a value.
  //! @param value  Floating-point value.
  //! @return A float value representing the largest integer that is
  ...
  //! @return The square root of argument, or an indefinite if
  //! argument is negative.
  static _float Sqrt(_float value);
  //! Calculates the exponential.
  //! @param value  Floating-point value.
  //! @return The exponential value of argument, or INF if overflow,
  //! or 0 if underflow.
  static _float Exp(_float value);
  //! Calculates logarithms.
  //! @param value  Value whose logarithm is to be found.
  //! @return The logarithm of argument, or an indefinite if argument
  //! is negative, or INF if argument is 0.
  ...
  //! @param radian  Angle in radians.
  //! @param r   The value of sin and cos
  //! {sin(x), cos(x)}.
  //! @return none.
  ...
  //! @return The arcsine of argument in the range -PI/2 to PI/2
  //! radians.
  static _float Asin(_float value);
  //! Calculates the arccosine.
  //! @param value  Value between -1 and 1 whose arccosine
  //! is to be calculated.
  //! @return The arccosine of argument in the range 0 to PI radians.
  static _float Acos(_float value);
  //! Calculates the arctangent.
  //! @param value  Value whose arctangent is to be
  //! calculated.
  //! @return The arctangent of argument in the range -PI/2 to PI/2
  //! radians, or 0 if value is 0.
  static _float Atan(_float value);
  //! Calculates the arctangent.
  //! @param valuey  Value whose arctangent is to be
  //! calculated.
  //! @param valuex  Value whose arctangent is to be
  //! calculated.
...
template <typename Type>
inline Type Math::Remap(Type value, Type input_min, Type input_max,
                        Type output_min, Type output_max, _ubool clamp) {
  ...
  if (i_min == i_max) return o_min;

  if (clamp) value = Clamp(value, i_min, i_max);
...
inline _int Math::Abs(_int value) { return value >= 0 ? value : -value; }

inline _float Math::Abs(_float value) { return value >= 0.0f ? value : -value; }
...
inline _ubool Math::IsPrime(_dword value) {
  if (value < 2) return _false;

  if (value == 2) return _true;

  for (_dword i = 2; i * i <= value; i += 2) {
    if (value % i == 0) return _false;
  }
...
inline _float Math::Ceil(_float value) { return (_float)ceil(value); }

inline _float Math::Floor(_float value) { return (_float)floor(value); }
...
inline _float Math::Sqrt(_float value) {
  ...
  return value*u.x*(1.5f - xhalf*u.x*u.x);// Newton step, repeating increases
  accuracy
  */
  return (_float)::sqrt(value);
}

inline _float Math::Exp(_float value) { return (_float)exp(value); }

inline _float Math::Log(_float value) { return (_float)log(value); }
...
inline _float Math::Sin(_float radian) { return (_float)sin(radian); }

inline _float Math::Cos(_float radian) { return (_float)cos(radian); }
...
inline _float Math::Tan(_float radian) { return (_float)tan(radian); }

inline _float Math::Cot(_float radian) { return 1.0f / (_float)tan(radian); }

inline _float Math::Asin(_float value) {
  if (value > 1.0f) return cPi / 2.0f;
...
inline _float Math::Acos(_float value) {
  ...
  if (value < -1.0f) return cPi;

  return (_float)acos(value);
}

inline _float Math::Atan(_float value) { return (_float)atan(value); }
