...
StringType Path::GetFileName(StringPtrType path, _ubool extension) {
	...
	// Copy the file name
	StringType string;
	string.CopyString(path.CStr() + (i + 1), length);
...
StringType Path::RemoveExtension(StringPtrType path) {
	...
	StringType string;
	string.CopyString(path.CStr(), length);
...
_ubool Path::DoesMatchExtension(AStringPtr filename, AStringPtr extension_list, _ubool ignorecase) {
	...
	Platform::AnsiToUtf16(filename_unicode, 1024, filename.CStr());

	_charw extension_list_unicode[1024];
	Platform::AnsiToUtf16(extension_list_unicode, 1024, extension_list.CStr());
