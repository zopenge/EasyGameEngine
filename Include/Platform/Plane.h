...
public:
	...
	inline Plane( const Vector3& vector1, const Vector3& vector2, const Vector3& vector3 );
...
Matrix4 Plane::GetReflectionMatrix( ) const
{
	...
	bc = b * c;
	return Matrix4(
		-2.0f * a * a + 1.0f,	-2.0f * ab,				-2.0f * ac,				0.0f,
		-2.0f * ab,				-2.0f * b * b + 1.0f,	-2.0f * bc,				0.0f,
		-2.0f * ac,				-2.0f * bc,				-2.0f * c * c + 1.0f,	0.0f,
...
{
	if ( Math::Abs( d - plane.d ) > degree_tolerance / 100.0f )
		return _false;
	...
	if ( Math::Abs( angle ) > Math::Degree2Radian( degree_tolerance ) )
		return _false;
