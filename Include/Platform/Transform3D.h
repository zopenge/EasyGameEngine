...
public:
	//! Constructor, create a Transform3D without sets rotation and translate components.
	//! @param		none
	inline Transform3D( );
...
public:
	//! Multiply a Transform3D to current Transform3D, and save result in current one.
	...
	//! Multiply a Transform3D to current Transform3D, return a new Transform3D to save result.
	//! @param		Transform3D		The Transform3D to multiply.
	//! @return		The result Transform3D of multiplication.
	...
	static Transform3D Lerp( const Transform3D& transform1, const Transform3D& transform2, _float factor );
};
...
{
	if ( mTranslate != transform.mTranslate )
		return _false;

	if ( !mRotation.Equal( transform.mRotation ) )
		return _false;
