...
inline Vector3::Vector3(_float xx, _float yy, _float zz)
    : x(xx), y(yy), z(zz) {
}
...
inline _ubool Vector3::Equal(const Vector3& vector, _float margin) const {
	...
	if (Math::Abs(z - vector.z) > margin) return _false;
...
inline Vector3 Vector3::Clamp(const Vector3& vector, const Vector3& min,
                              const Vector3& max) {
...
inline Vector3& Vector3::NormalizeByBoundingBox(const Vector3& bounding_min,
                                                const Vector3& bounding_max) {
...
inline Vector3 Vector3::Project(const Vector3& vector1,
...
inline Vector3 Vector3::Lerp(const Vector3& vector1, const Vector3& vector2,
                             _float factor) {
	return vector1 + (vector2 - vector1) * factor;
}

inline Vector2 Vector3::Convert3DTo2D(const PointU& viewport_size,
                                      const Vector3& point) {
	return Vector2((1.0f + point.x) * viewport_size.x * 0.5f,
	               (1.0f - point.y) * viewport_size.y * 0.5f);
}
