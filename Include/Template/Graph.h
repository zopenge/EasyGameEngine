...
public:
	...
	GraphNode( const Type& element, const Key& key ) : mElement( element ), mKey( key )
	{
		mConnectionID = 0;
	}
	//! Copy-Constructor, create a graph node by copy from another one.
	//! @param		node		The other graph node.
	GraphNode( const GraphNode< Type, Edge, Key >& node ) : mElement( node.mElement ), mKey( node.mKey ), mConnections( node.mConnections )
	{
