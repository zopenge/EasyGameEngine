...
public:
	...
	struct IteratorBase
	{
		...
		IteratorBase( _int index_of_nodes = -1, _int index_of_nodes_array = -1, HashMap* hash_map = _null ) 
			: mIndexOfNodes( index_of_nodes ), mNodesArrayIndex( index_of_nodes_array ), mHashMap( hash_map ) { }

		//! Check the iterator if it is valid ( points to an available element ).
		//! @param		none
		//! @return		True if the iterator is valid, false otherwise.
		_ubool IsValid( ) const
			{ return mIndexOfNodes != -1; }

		//! Type conversion, get the key reference.
		//! @param		none
		//! @return		Reference of the key.
		Key& GetKey( )
			{ return mHashMap->mNodeArrayChunks[ mNodesArrayIndex ][ mIndexOfNodes ].mObject1; }
		//! Type conversion, get the key reference.
		//! @param		none
		//! @return		Reference of the key.
		...
		Type& GetObject( )
			{ return *mHashMap->mNodeArrayChunks[ mNodesArrayIndex ][ mIndexOfNodes ].mObject2; }
		//! Type conversion, get the object reference.
		//! @param		none
		//! @return		Reference of the object.
	...
	struct Iterator : public IteratorBase
	{
		...
		_void operator -- ( _int )
		{
			if ( this->mIndexOfNodes == -1 )
				return;

			if ( this->mIndexOfNodes == 0 )
			{
				if ( this->mNodesArrayIndex == 0 )
				{
					this->mIndexOfNodes = -1;
				}
				else
				{
					while ( this->mNodesArrayIndex >= 0 )
					{
						if ( this->mHashMap->mNodeArrayChunks[ -- this->mNodesArrayIndex ].Number( ) > 0 )
						{
							...
							this->mIndexOfNodes = this->mHashMap->mNodeArrayChunks[ this->mNodesArrayIndex ].Number( ) - 1;
							return;
						}
					}
		...
		_void operator ++ ( _int )
		{
			...
			if ( this->mIndexOfNodes == (_int)this->mHashMap->mNodeArrayChunks[ this->mNodesArrayIndex ].Number( ) - 1 )
			{
