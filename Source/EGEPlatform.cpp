...
{
	EGE::_void* buffer = EGE::Memory::GetInstance( ).Alloc( number * size, filename, linenumber );
	if ( buffer != _null )
...
{
	return EGE::Memory::GetInstance( ).Realloc( pointer, size, filename, linenumber );
}
