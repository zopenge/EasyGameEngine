//! @file     ImageFillerComplexBookshelf.cpp
//! @author   LiCode
//! @version  1.0.0.613
//! @date     2011/01/17
...
ImageFillerComplexBookshelf::ImageFillerComplexBookshelf( const PointU& size, const PointU& space ) : BaseClass( size, space )
{
	// All the region can be filled
	mRegionMaskBuffer = new _byte[ mFreeArea ];
	Platform::MemSet( mRegionMaskBuffer, 0, mFreeArea );

	// We have one base line
	mBaseLines.Append( 0 );
}

ImageFillerComplexBookshelf::~ImageFillerComplexBookshelf( )
{
	Clear( );
}

_void ImageFillerComplexBookshelf::UpdateRegionMaskBuffer( const RectU& rect )
{
	for ( _dword y = rect.t; y < rect.t + rect.GetHeight( ); y ++ )
		Platform::MemSet( mRegionMaskBuffer + y * mSize.x + rect.l, 1, rect.GetWidth( ) );
}

_ubool ImageFillerComplexBookshelf::TestRegionMaskBuffer( const RectU& rect ) const
{
	for ( _dword y = rect.t; y < rect.t + rect.GetHeight( ); y ++ )
	{
...
_dword ImageFillerComplexBookshelf::GetFreeXStartPosition( _dword y )
{
...
_void ImageFillerComplexBookshelf::CleanupBaseLines( )
{
...
_ubool ImageFillerComplexBookshelf::OnFillImage( _dword width, _dword height, RectU& rect )
{
	// Start to fill image
...
_void ImageFillerComplexBookshelf::Clear( )
{
	BaseClass::Clear( );
