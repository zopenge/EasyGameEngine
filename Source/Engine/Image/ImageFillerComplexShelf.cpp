//! @file     ImageFillerComplexShelf.cpp
//! @author   LiCode
//! @version  1.0.0.613
//! @date     2011/01/17
//! Copyright ...
...
//----------------------------------------------------------------------------
// ImageFillerComplexShelf Implementation
//----------------------------------------------------------------------------

ImageFillerComplexShelf::ImageFillerComplexShelf( const PointU& size, const PointU& space ) : BaseClass( size, space )
{
...
ImageFillerComplexShelf::~ImageFillerComplexShelf( )
{
	Clear( );
}

_void ImageFillerComplexShelf::UpdateRegionMaskBuffer( const RectU& rect )
{
	for ( _dword y = rect.t; y < rect.t + rect.GetHeight( ); y ++ )
		Platform::MemSet( mRegionMaskBuffer + y * mSize.x + rect.l, 1, rect.GetWidth( ) );
}

_ubool ImageFillerComplexShelf::TestRegionMaskBuffer( const RectU& rect ) const
{
	for ( _dword y = rect.t; y < rect.t + rect.GetHeight( ); y ++ )
	{
...
_dword ImageFillerComplexShelf::GetFreeXStartPosition( _dword y )
{
...
_void ImageFillerComplexShelf::CleanupBaseLines( )
{
...
_ubool ImageFillerComplexShelf::OnFillImage( _dword width, _dword height, RectU& rect )
{
	// Start to fill image
...
_void ImageFillerComplexShelf::Clear( )
{
	BaseClass::Clear( );
