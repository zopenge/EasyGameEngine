...
winProcessDebugger::~winProcessDebugger( )
{
	// Terminate the main process ( when we reach here then indicates the process run the failure code )
	if ( mProcessHandle != _null )
		Platform::TerminateProcess( mProcessHandle, -1 );
}
...
_ubool winProcessDebugger::RunProcess( )
{
	...
	while ( ::WaitForDebugEvent( &mDebugEvent, INFINITE ) )
	{
			...
			case EXCEPTION_DEBUG_EVENT:
			{
				...
				if ( mDebugEvent.u.Exception.ExceptionRecord.ExceptionCode != EXCEPTION_BREAKPOINT )
				{
					...
					if ( mDebugEvent.u.Exception.dwFirstChance == 0 )
					{
						...
						event.mCodeString	= Exception::GetExceptionCodeString( event.mCodeID );
						NotifyObservers( this, event );
					}
				}
		...
		if ( ::ContinueDebugEvent( mDebugEvent.dwProcessId, mDebugEvent.dwThreadId, continue_status ) == _false )
			return _false;
	}
...
_ubool winProcessDebugger::Run( WStringPtr modulename, WStringPtr cmdline, _dword creationflags, WStringPtr workdir, _ubool nodebugheap, _ubool fastmode )
{
	...
	// Get the module name (UNICODE)
	const _charw* modulename_unicode = _null;
	if ( modulename.IsEmpty( ) == _false )
		modulename_unicode = modulename.Str( );

	// Create process in UNICODE mode
	if ( Platform::CreateProcess( modulename_unicode, cmdline.Str( ), creationflags, workdir.Str( ), &mProcessHandle ) == _false )
		return _false;
