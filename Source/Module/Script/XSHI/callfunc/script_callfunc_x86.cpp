...
//----------------------------------------------------------------------------
// ScriptCallFunc:x86 Implementation
//----------------------------------------------------------------------------
...
_qword ScriptCallFunc::CallNativeFunc( VMXSHI* vm, const NativeFunctionInfoXSHI& function_info, _void* object, _dword* args )
{
	...
	// Call it
	switch ( function_info.mCallConv )
	{
		case _XICC_CDECL:
		{
			ret = CallCDeclFunctionQWord( args, function_info.mParamSize << 2, (size_t) function_info.mFuncPointer );
		}
		break;

		case _XICC_STDCALL:
		{
			ret = CallSTDCallFunctionQWord( args, function_info.mParamSize << 2, (size_t) function_info.mFuncPointer );
		}
		break;

		case _XICC_THISCALL:
		{
			ret = CallThisCallFunctionQWord( object, args, function_info.mParamSize << 2, (size_t) function_info.mFuncPointer );
		}
		break;

		case _XICC_VIRTUAL_THISCALL:
		{
			// Get virtual function table from the object pointer
			_dword* vftable = *(_dword**) object;

			ret = CallThisCallFunctionQWord( object, args, function_info.mParamSize << 2, vftable[ size_t( function_info.mFuncPointer ) >> 2 ] );
		}
		break;

		case _XICC_CDECL_OBJLAST:
		{
			ret = CallCDeclFunctionQWordObjLast( object, args, function_info.mParamSize << 2, (size_t) function_info.mFuncPointer );
		}
		break;

		case _XICC_CDECL_OBJFIRST:
		{
			// Call the system object method as a cdecl with the obj ref as the first parameter
			ret = CallCDeclFunctionQWordObjFirst( object, args, function_info.mParamSize << 2, (size_t) function_info.mFuncPointer );
		}
		break;

		default:
			if ( vm != _null )
				gDynamicXSHI->SetVMInternalExceptionDesc( vm, _WIDEN( TXT_INVALID_CALLING_CONVENTION ) );
			break;
	}

	// If the return is a float value we need to get the value from the FP register
	if ( function_info.HasFlags( NativeFunctionInfoXSHI::_FLAG_HOST_RETURN_FLOAT ) )
	{
		if ( function_info.mHostReturnSize == 4 )
