...
ColliderBox2D::ColliderBox2D(const Vector2& lt, const Vector2& rt, const Vector2& rb, const Vector2& lb) {
	mCorner[0] = lt;
	mCorner[1] = rt;
	mCorner[2] = rb;
	mCorner[3] = lb;
...
ColliderBox2D::ColliderBox2D(const Vector2& center, _float w, _float h, _float radian) {
...
ColliderBox2D::ColliderBox2D(const ColliderBox2D& box) {
...
_void ColliderBox2D::ComputeAxes() {
...
RectF ColliderBox2D::ToRectF() const {
...
ColliderBox2D& ColliderBox2D::FromRectF(const RectF& rect) {
	*this = ColliderBox2D(rect.GetCenter(), rect.GetWidth(), rect.GetHeight(), 0.0f);

	return *this;
}

_void ColliderBox2D::MoveTo(const Vector2& center) {
...
_void ColliderBox2D::SetSize(_float width, _float height) {
...
_ubool ColliderBox2D::Overlaps1Way(const ColliderBox2D& other) const {
	for (int a = 0; a < 2; ++a) {
...
_ubool ColliderBox2D::PointInOrientedBox(const Vector2& point) const {
...
ColliderBox2D ColliderBox2D::operator*(const Matrix3& matrix) const {
	ColliderBox2D returnbox(*this);

	return returnbox *= matrix;
}

ColliderBox2D& ColliderBox2D::operator*=(const Matrix3& transform) {
	mCorner[0] *= transform;
	mCorner[1] *= transform;
	mCorner[2] *= transform;
	mCorner[3] *= transform;
