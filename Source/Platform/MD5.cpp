...
MD5Code MD5::BuildFromString(AStringPtr string) {
	return BuildFromBuffer((const _byte*)string.CStr(), string.SizeOfBytes() - sizeof(_chara));
}

MD5Code MD5::BuildFromString(WStringPtr string) {
	return BuildFromBuffer((const _byte*)string.CStr(), string.SizeOfBytes() - sizeof(_charw));
}
