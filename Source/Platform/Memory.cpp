...
static int OnCRTAllocHookCallback( int nAllocType, void *userData, size_t size, int nBlockType, long requestNumber, const unsigned char* filename, int lineNumber )
{
	...
	switch ( nAllocType )
	{
		case _HOOK_ALLOC:
		{
			if ( filename != _null )
			{
				if ( filename_ansi == "wincore.cpp" 
					|| filename_ansi == "plex.cpp" 
					|| filename_ansi == "strcore.cpp" 
					|| filename_ansi == "oleinit.cpp" 
...
_void Memory::OnOutputStringCallback( _dword id, const _byte* address, const MemoryAllocation& allocation, const QwordParameters2& parameters )
{
	...
	_chara dumpstringbuffer[8192]; dumpstringbuffer[0] = 0;
	...
	if ( address != _null )
	{
		_chara bufferdata[1024]; bufferdata[0] = 0;
...
_ubool Memory::IsEnableFullCallstackCheck( _dword line_number, const _chara* file_name ) const
{
	if ( mIsUseFullCallstackCheck == _false )
		return _false;
	...
	if ( mFileNameFilter[0] != 0 )
	{
		...
		if ( Platform::SearchL2R( file_name, mFileNameFilter ) == -1 )
			return _false;
	}

	if ( mLineNumberFilter != 0 && mLineNumberFilter != line_number )
		return _false;

	return _true;
}

_void Memory::DumpAllocations( _dword min_size, OnOutputString funcpointer, const AllocationLink& allocations, const QwordParameters2& parameters )
{
	// No any memory blocks
	if ( allocations.Number( ) == 0 )
		return;
...
_void Memory::DumpAllocations( _dword min_size, OnOutputString funcpointer, const BlockAllocationArray& allocations, const QwordParameters2& parameters )
{
	// No any memory blocks
	if ( allocations.Number( ) == 0 )
		return;
...
_void Memory::AllocBlock( _dword block_number, _dword size, const _chara* filename, _dword linenumber )
{
#ifdef _DEBUG
...
_void* Memory::Alloc( _dword size, const _chara* filename, _dword linenumber )
{
	...
	{
		node->mElement.mSrcFileLines	= (ASrcFileLineInfo*) Platform::GlobalAlloc( sizeof( ASrcFileLineInfo ) * mMaxFrameNumber );
...
{
	if ( buffer == _null )
		return Alloc( size, filename, linenumber );
...
_void Memory::Dump( _dword min_size, OnOutputString funcpointer, const QwordParameters2& parameters )
{
...
_void Memory::DumpLeakInfo( )
{
	...
	File log_file;
	if ( log_file.Open( L"memory_leak.log", _FILE_CREATE_ALWAYS, _FILE_OPERATION_WRITE, _FILE_SHARE_READ ) == _false )
		return;
