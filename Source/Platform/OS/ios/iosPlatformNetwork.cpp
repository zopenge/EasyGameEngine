...
{
	_socket handle = anyPlatformNetwork::CreateSocket( families, type, block_mode );
	if ( handle == _null )
		return _null;

	// Ignore the SIGPIPE signal to prevent crash
	anyPlatformNetwork::SetSocketOperationStatus( handle, SOL_SOCKET, SO_NOSIGPIPE, 1 );
