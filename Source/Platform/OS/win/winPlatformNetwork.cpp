...
DomainFamilyType Platform::GetFamilyType(_dword port, const _chara* url_address) {
	return anyPlatformNetwork::GetFamilyType(port, url_address);
}

_socket Platform::CreateSocket(DomainFamilyType families, SocketType type, _ubool block_mode) {
	return anyPlatformNetwork::CreateSocket(families, type, block_mode);
}

_socket Platform::CreateListenedSocket(DomainFamilyType families, SocketType type, _ubool block_mode, _dword port, _dword max_connection_number) {
