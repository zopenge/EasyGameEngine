...
_dword PersistenceThread::OnRunThread( const QwordParameters2& parameters )
{
	_dword index = 0;
...
_void PersistenceThread::OnCloseThread( _dword exitcode, const QwordParameters2& parameters )
{
...
_ubool PersistenceThread::Create( _dword max_updaters_number, _dword priority, _ubool suspend, _dword interval, WStringPtr name )
{
	...
	if ( Thread::Create( priority, suspend, name, QwordParameters2::cNull ) == _false )
		return _false;
...
_ubool PersistenceThread::AddUpdaterFunc( OnUpdateFunction func, const QwordParameters2& parameters )
{
