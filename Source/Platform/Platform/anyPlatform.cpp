...
*/
//!	I copy the code from 'http://www.strudel.org.uk/itoa/', thanks for sharing it.
...
{																						\
	if ( base < 2 || base > 36 ) { *result = (CharType) ( '\0' ); return result; }		\
	CharType* ptr = result, *ptr1 = result, tmp_char;									\
	ValueType tmp_value;																\
	do {																				\
		tmp_value = value;																\
		value /= base;																	\
		*ptr++ = (CharType) ( "zyxwvutsrqponmlkjihgfedcba9876543210123456789abcdefghijklmnopqrstuvwxyz"[35 + ( tmp_value - value * base )] );		\
	}																					\
	while ( value );																	\
...
//!	I copy the code from 'http://www.codeforge.com/read/185702/UTOA.C__html', thanks for sharing it.
...
{																						\
	...
	{																					\
		result[i++] = (CharType) ( "0123456789abcdefghijklmnopqrstuvwxyz"[value % base] );	\
	}																					\
	while ( value /= base );															\
...
#if defined(_PLATFORM_CHROME_) && defined(NACL)
extern "C" {
...
_ubool anyPlatform::Initialize( )
{
	...
	EGE_STATIC_ASSERT( sizeof( _crcvalue ) == 4, "Basic data type size check" );
	...
	OUTPUT_DEBUG_STRING( FORMAT_ASTRING_1( "EGE: '_long' type size: %d\n", (_dword)sizeof( _long ) ) );
	OUTPUT_DEBUG_STRING( FORMAT_ASTRING_2( "EGE: BuildTime: %s %s\n", _BUILDDATE_A, _BUILDTIME_A ) );
	OUTPUT_DEBUG_STRING( FORMAT_ASTRING_1( "EGE: MainThread: 0x%.16llx\n", gMainThreadID ) );
