...
_handle anyPlatformProcess::CreateNamedPipe( const _charw* name, _PIPE_ACCESS accessmode, _dword mode, _dword maxnumber, _dword outbuffersize, _dword inbuffersize, _dword timeout )
{
	return _null;
}
...
_ubool anyPlatformProcess::SetThreadPriority( _handle thread, _PRIORITY priority )
{
	return _false;
}
...
_handle anyPlatformProcess::CreateThread( Platform::OnThreadStartRoutine funcpointer, _dword priority, _void* parameter, _ubool suspend, _qword* threadid )
{
#if (USE_PTHREAD == 1)
	// Create thread info
	...
	// (As the numbers imply, real-time threads always have higher priority than normal threads.)
#ifndef NACL
	if ( priority != 0 )
	{
		sched_param param;
	...
	// Create thread
	int err = _pthread_create( thread_info, &attr, (OnFuncPointer)funcpointer, parameter );
	if ( err != 0 )
	{
		EGE_DELETE( thread_info );
		_pthread_attr_destroy( &attr );

		OUTPUT_DEBUG_STRING( FORMAT_ASTRING_1( "Create thread failed, err: %d\n", err ) );
		return _null;
	}
	...
	OUTPUT_DEBUG_STRING( FORMAT_ASTRING_1( "Create thread OK, ID: 0x%.16llx\n", thread_id ) );
...
_ubool anyPlatformProcess::WaitThread( _handle thread, _dword* ret_code )
{
	...
	_dword exit_code = 0;
	if ( _pthread_join( *thread_info, (_void**) &exit_code ) != 0 )
		return _false;
