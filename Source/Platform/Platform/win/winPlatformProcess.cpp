...
static _dword TranslateThreadPriority( _PRIORITY priority )
{
 switch ( priority )
 {
  case _PRIORITY_LOWEST:   return THREAD_PRIORITY_LOWEST;
  case _PRIORITY_BELOW_NORMAL: return THREAD_PRIORITY_BELOW_NORMAL;
  case _PRIORITY_NORMAL:   return THREAD_PRIORITY_NORMAL;
  case _PRIORITY_ABOVE_NORMAL: return THREAD_PRIORITY_ABOVE_NORMAL;
  case _PRIORITY_HIGHEST:   return THREAD_PRIORITY_HIGHEST;
  default:
   return 0;
 }
}
...
_void Platform::RestartCurrentProcess( )
{
 ...
 AString cmd_string = "start " + AString( ).FromString( _ENCODING_UTF16, module_file_name );
 system( cmd_string.Str( ) );
