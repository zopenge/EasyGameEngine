...
Time::Time( UStringPtr string )
{
 ...
 StringFormatter::ParseString( string.Str( ), "%d/%d/%d %d:%d:%d(%d)", 
...
{
 // The string format is "YYYY/MM/DD HH::MM::SS(MMMM)"
 StringFormatter::ParseString( string.Str( ), L"%d/%d/%d %d:%d:%d(%d)", 
  mYear, mMonth, mDayOfMonth, mHour, mMinute, mSecond, mMilliseconds );
}
...
{
 _charw buffer_unicode[1024]; buffer_unicode[0] = 0;
...
{
 WString tag_name; _word day, year, hour, minute, second; AString week, mon;
 StringFormatter::ParseString( string.Str( ), "%s: %s, %d %s %d %d:%d:%d GMT", tag_name, week, day, mon, year, hour, minute, second );
