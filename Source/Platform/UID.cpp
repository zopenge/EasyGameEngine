...
UID128& UID128::FromString(AStringPtr string) {
	*this = UID::String2UUID(string.CStr());
	return *this;
}
...
UID128 UID::String2UUID(const _charw* uidstring) {
	...
	WString substring[6];
	substring[0] = WString(uidstring, 8);
	uidstring += 9; // contains '-' character
	substring[1] = WString(uidstring, 4);
	uidstring += 5; // contains '-' character
	substring[2] = WString(uidstring, 4);
	uidstring += 5; // contains '-' character
	substring[3] = WString(uidstring, 4);
	uidstring += 5; // contains '-' character
	substring[4] = WString(uidstring, 4);
	uidstring += 4;
